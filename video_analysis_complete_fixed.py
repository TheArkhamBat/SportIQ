...
class VideoAnalyzer:
    ...
    def visualize_results(self, video_path):
        ...
        # Get skeleton connections if available
        skeleton_conns = []
        if hasattr(self.pose_estimator, 'skeleton_connections'):
            skeleton_conns = self.pose_estimator.skeleton_connections
        else:
        ...
        while True:
            ...
            if frame_idx < len(self.all_poses):
                ...
                if frame_idx < len(self.all_angles):
                    ...
                    cv2.putText(frame, f"INJURY RISK: {risk['overall_risk'][:30]}", 
                              (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                    cv2.putText(frame, f"Frame: {frame_idx}/{len(self.all_poses)}", 
